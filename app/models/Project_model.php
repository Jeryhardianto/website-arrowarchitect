...
class Project_model
{
    ...
    {
        $this->db->query('SELECT id,nama_project, location, gambar  FROM project ORDER BY id DESC');
        return $this->db->resultSet();
    }

    public function getAllDetailProject($id)
    {
        $this->db->query('SELECT id, foto  FROM detail_project WHERE id_project=:id ORDER BY id DESC');
        $this->db->bind('id', $id);
        return $this->db->resultSet();
    }
    ...
    {
        $this->db->query('SELECT id, foto  FROM detail_project WHERE id = :id');
        $this->db->bind('id', $id);
        return $this->db->resultSet();
    }
    ...
    {
        $this->db->query('SELECT id,foto  FROM detail_project WHERE id_project = :id');
        $this->db->bind('id', $id);
        return $this->db->resultSet();
    }
    ...
    {
        $this->db->query('SELECT nama_project, location, gambar   FROM project WHERE id=:id');
        $this->db->bind('id', $id);
        return $this->db->resultSet();
    }
    ...
    {
        $query = "UPDATE detail_project SET foto=:foto, date_update=:tglUpdate
      WHERE id=:id";
    ...
    {
        $query = "UPDATE project SET nama_project=:nama_project,gambar=:gambar, location=:location, date_update=:tglUpdate
      WHERE id=:id";
